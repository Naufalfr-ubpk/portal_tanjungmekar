<x-guest-layout>
    ...
    <div class="flex flex-col items-center mb-6 mt-8 sm:mt-0">
        ...
        <h2 class="mt-4 text-2xl font-bold text-[#0E4D2B]">Login Portal</h2>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div>
            ...
            <x-text-input id="email" class="block mt-1 w-full border-[#A5D6A7] focus:border-[#2E7D32] focus:ring-[#2E7D32]" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" />
        ...
        <div class="mt-4" x-data="{ show: false }">
            ...
            <div class="relative">
                <x-text-input id="password" x-bind:type="show ? 'text' : 'password'" class="block mt-1 w-full border-[#A5D6A7] focus:border-[#2E7D32] focus:ring-[#2E7D32] pr-10" name="password" required autocomplete="current-password" />
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" class="rounded border-gray-300 text-[#2E7D32] shadow-sm focus:ring-[#2E7D32]" name="remember">
        ...
        <div class="flex items-center justify-between mt-6">
            @if (Route::has('password.request'))
                <a class="underline text-sm text-gray-600 hover:text-[#2E7D32] rounded-md focus:outline-none" href="{{ route('password.request') }}">Lupa Password?</a>
            @endif
            <button type="submit" class="inline-flex items-center px-6 py-2 bg-[#0E4D2B] border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-[#2E7D32] focus:outline-none transition ease-in-out duration-150">Login</button>
        </div>
        ...
        <p class="mt-6 text-center text-sm text-gray-600">Belum punya akun? <a href="{{ route('register') }}" class="text-[#2E7D32] font-bold hover:underline">Sign Up</a></p>
    </form>
</x-guest-layout>