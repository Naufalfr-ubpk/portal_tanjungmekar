...
<body class="font-sans antialiased bg-gray-100 text-gray-900 flex" x-data="{ sidebarOpen: false }">
    ...
    <aside class="w-64 bg-[#0E4D2B] h-screen text-white flex flex-col shadow-xl fixed z-30 transform transition-transform duration-300 md:translate-x-0" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'">
        ...
        <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-2 custom-scrollbar">
            ...
            @if(Auth::user()->role === 'admin')
            <a href="{{ route('admin.laporan-web.index') }}" class="flex items-center gap-3 hover:bg-[#2E7D32] text-gray-200 hover:text-white px-4 py-3 rounded-lg font-semibold transition mb-4">
    ...
    <main class="flex-1 md:ml-64 bg-gray-50 min-h-screen transition-all duration-300">
            ...
            <div class="flex items-center gap-3 md:gap-4">
                ...
                <div x-data="{ openAdminProfile: false }" class="relative">
                    <button @click="openAdminProfile = !openAdminProfile" @click.away="openAdminProfile = false" class="flex items-center gap-2 px-2 md:px-4 py-2 bg-gray-50 border border-gray-200 rounded-full hover:bg-gray-100 transition focus:outline-none">
                        ...
                        <!-- Animasi Chevron -->
                        <svg :class="{'rotate-180': openAdminProfile}" class="w-4 h-4 text-gray-600 font-bold transition-transform duration-200 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                        </svg>
                    </button>
                    <div x-show="openAdminProfile" x-transition.opacity style="display: none;" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 border border-gray-200 z-50">
        ...
        <div class="p-4 md:p-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
               ...
               @if(Auth::user()->role === 'admin')
                <a href="{{ route('admin.laporan-web.index') }}" class="block bg-white rounded-2xl shadow-sm border border-gray-200 p-6 hover:shadow-md hover:-translate-y-1 transition transform">
